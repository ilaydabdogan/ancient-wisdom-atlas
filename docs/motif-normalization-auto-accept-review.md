...
Policy: auto-accept only high-confidence `map_to_existing_group` suggestions whose target group already exists. Everything else stays reviewable.

## Summary

- Suggestions reviewed: 700
- Auto-accepted into main taxonomy: 10
- Staged for review: 690

## Auto-Accepted

| Motif ID | Label | Group | Relationship | Occurrences | Traditions |
| --- | --- | --- | --- | ---: | --- |
| `food_theft_by_feigned_helpfulness` | Food theft by feigned helpfulness | `trickster` Trickster and Boundary Crosser | functional_variant | 1 | indigenous_australian |
| `forbidden_intoxicants_and_divinatory_objects_as_satanic_instruments` | Forbidden intoxicants and divinatory objects as Satanic instruments | `sacred_law` Sacred Law and Social Order | ritual_variant | 1 | islamic |
| `forbidden_opening_of_a_guarded_container_releases_destructive_forces` | forbidden opening of a guarded container releases destructive forces | `sacred_vessel` Sacred Vessel and Container | functional_variant | 1 | greek_roman |
| `ford_duel_as_crisis_point` | ford duel as crisis point | `sacred_combat` Sacred Combat and Ritual Warfare | narrower_than | 1 | celtic_irish |
| `foreknown_doom_disregarded_under_fate` | foreknown doom disregarded under fate | `death_by_fate` Death Attributed to Fate or Destiny | narrower_than | 1 | greek |
| `forged_abundance_object` | forged abundance object | `sacred_craft` Sacred Craft and Divinely Aided Construction | functional_variant | 1 | finnish_karelian |
| `forging_a_wondrous_object_from_small_or_unlikely_materials` | Forging a wondrous object from small or unlikely materials | `sacred_craft` Sacred Craft and Divinely Aided Construction | functional_variant | 1 | finnish_karelian |
| `forgiveness_preferred_over_proportionate_retaliation` | Forgiveness preferred over proportionate retaliation | `sacred_law` Sacred Law and Social Order | functional_variant | 1 | islamic |
| `funerary_body_retrieval_and_funeral_sequence` | Funerary body retrieval and funeral sequence | `heroic_funeral_rites` Heroic Funeral Rites and Memorialization | ritual_variant | 1 | greek |
| `generation_of_beings_from_a_primordial_body` | Generation of beings from a primordial body | `primordial_sacrifice` Primordial Sacrifice and Dismemberment | narrower_than | 1 | norse |
...
| `abandonment_of_the_corruptible_body` | Abandonment of the corruptible body | map_to_existing_group | `mystical_quest` Mystical Quest and Interior Journey | medium | not high-confidence existing-group placement |
...
| `abducted_infant_whose_fate_is_unknown` | abducted infant whose fate is unknown | map_to_existing_group | `miraculous_child` Miraculous Child and Sacred Birth | medium | not high-confidence existing-group placement |
| `abolition_of_artificial_standards_restores_natural_integrity` | abolition of artificial standards restores natural integrity | map_to_existing_group | `duality` Duality and Cosmic Opposition | medium | not high-confidence existing-group placement |
| `abolition_of_coercive_widow_inheritance` | abolition of coercive widow inheritance | map_to_existing_group | `sacred_law` Sacred Law and Social Order | medium | not high-confidence existing-group placement |
...
| `absent_champion_invoked_to_restore_courage` | absent champion invoked to restore courage | map_to_existing_group | `divine_warrior` Divine Warrior and Sacred Combat | medium | not high-confidence existing-group placement |
| `absent_protectors_invoked_in_crisis` | absent protectors invoked in crisis | map_to_existing_group | `divine_intervention` Divine Intervention and Guidance | medium | not high-confidence existing-group placement |
...
| `absurd_exchange_value_reveals_corruption` | Absurd exchange value reveals corruption | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
| `absurd_valuation_as_moral_exposure` | absurd valuation as moral exposure | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
...
| `abundant_faraway_land` | abundant faraway land | map_to_existing_group | `otherworld` Otherworld and Enchanted Realm | medium | not high-confidence existing-group placement |
...
| `acceptance_of_destiny_after_loss` | acceptance of destiny after loss | map_to_existing_group | `death_by_fate` Death Attributed to Fate or Destiny | medium | not high-confidence existing-group placement |
...
| `accidental_encounter_with_supernatural_beings_in_a_wild_place` | Accidental encounter with supernatural beings in a wild place | map_to_existing_group | `otherworld` Otherworld and Enchanted Realm | medium | not high-confidence existing-group placement |
...
| `accumulation_of_wonderful_objects` | accumulation of wonderful objects | map_to_existing_group | `sacred_treasures` Sacred Treasures and Power Objects | medium | not high-confidence existing-group placement |
...
| `admired_feature_becomes_cause_of_ruin` | admired feature becomes cause of ruin | map_to_existing_group | `pride_and_downfall` Pride, Hubris, and Downfall | medium | not high-confidence existing-group placement |
...
| `adversarial_being_followed_by_the_unfaithful_majority` | Adversarial being followed by the unfaithful majority | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
| `afflicted_people_promised_succession_in_the_land` | Afflicted people promised succession in the land | map_to_existing_group | `covenant` Covenant and Sacred Bond | medium | not high-confidence existing-group placement |
...
| `aging_singer_laments_loss_of_voice` | Aging singer laments loss of voice | map_to_existing_group | `lament_and_mourning` Lament, Mourning, and Tragic Recognition | medium | not high-confidence existing-group placement |
| `alarm_pursuit_and_defeat_after_theft` | alarm, pursuit, and defeat after theft | map_to_existing_group | `trickster` Trickster and Boundary Crosser | medium | not high-confidence existing-group placement |
| `allegorical_figures_explained_by_a_host` | Allegorical figures explained by a host | map_to_existing_group | `sacred_knowledge` Sacred Knowledge and Wisdom | medium | not high-confidence existing-group placement |
...
| `ally_dies_or_nearly_dies_for_the_captive` | ally dies or nearly dies for the captive | map_to_existing_group | `sacrifice` Sacrifice and Offering | medium | not high-confidence existing-group placement |
...
| `altruistic_reception_of_refugees_by_fellow_believers` | altruistic reception of refugees by fellow believers | map_to_existing_group | `sacred_exchange` Sacred Exchange and Reciprocity | medium | not high-confidence existing-group placement |
...
| `ambitious_courtier_betrays_benefactor` | Ambitious courtier betrays benefactor | map_to_existing_group | `pride_and_downfall` Pride, Hubris, and Downfall | medium | not high-confidence existing-group placement |
...
| `ambush_overcome_with_lone_survivor` | ambush overcome with lone survivor | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
...
| `angelic_bearer_offers_transformative_drink` | angelic bearer offers transformative drink | map_to_existing_group | `mystical_quest` Mystical Quest and Interior Journey | medium | not high-confidence existing-group placement |
...
| `angelic_messengers_with_multiple_wings` | Angelic messengers with multiple wings | map_to_existing_group | `divine_intervention` Divine Intervention and Guidance | medium | not high-confidence existing-group placement |
| `angelic_obedience_and_restricted_intercession` | Angelic obedience and restricted intercession | map_to_existing_group | `sacred_law` Sacred Law and Social Order | medium | not high-confidence existing-group placement |
...
| `angelic_prostration_before_a_newly_created_human` | Angelic prostration before a newly created human | map_to_existing_group | `cosmic_origin` Cosmic Origin and Creation | medium | not high-confidence existing-group placement |
...
| `annihilating_breath_reduces_bodies_to_ashes` | annihilating breath reduces bodies to ashes | map_to_existing_group | `divine_warrior` Divine Warrior and Sacred Combat | medium | not high-confidence existing-group placement |
...
| `anonymous_champion_defeats_repeated_challengers` | Anonymous champion defeats repeated challengers | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
...
| `apostasy_and_replacement_of_a_religious_community` | apostasy and replacement of a religious community | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
| `apotropaic_diversion_of_predator_hunger` | apotropaic diversion of predator hunger | map_to_existing_group | `sacred_law` Sacred Law and Social Order | medium | not high-confidence existing-group placement |
...
| `appearance_of_christ_in_human_form_with_identity_proclamation` | Appearance of Christ in human form with identity proclamation | map_to_existing_group | `theophany` Theophany and Divine Manifestation | medium | not high-confidence existing-group placement |
...
| `appointment_of_twelve_authorized_representatives` | appointment of twelve authorized representatives | map_to_existing_group | `royal_legitimacy` Royal Legitimacy and Sacred Kingship | medium | not high-confidence existing-group placement |
| `arch_rebel_s_cruel_public_end_after_long_resistance` | arch-rebel's cruel public end after long resistance | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
...
| `armed_founder_succeeds_where_unarmed_founder_fails` | Armed founder succeeds where unarmed founder fails | map_to_existing_group | `culture_hero` Culture Hero and Founder | medium | not high-confidence existing-group placement |
| `armed_watcher_defeats_supernatural_seeming_intruder` | armed watcher defeats supernatural-seeming intruder | map_to_existing_group | `threshold_guardian` Threshold Guardian | medium | not high-confidence existing-group placement |
| `arming_before_a_liminal_night_mission` | arming before a liminal night mission | map_to_existing_group | `hero_journey` The Hero's Journey | medium | not high-confidence existing-group placement |
| `army_encamped_at_liminal_shore` | army encamped at liminal shore | map_to_existing_group | `sacred_waters` Sacred Waters and Passage Through Water | medium | not high-confidence existing-group placement |
...
| `astral_intermediaries_beneath_a_supreme_deity` | Astral intermediaries beneath a supreme deity | map_to_existing_group | `supreme_ruler` Supreme Ruler and Cosmic Sovereignty | medium | not high-confidence existing-group placement |
| `astral_remnant_of_a_beheaded_deer` | Astral remnant of a beheaded deer | map_to_existing_group | `death_and_transformation` Death, Rebirth, and Transformation | medium | not high-confidence existing-group placement |
...
| `auspicious_flame_omen` | Auspicious flame omen | map_to_existing_group | `sacred_fire` Sacred Fire and Divine Light | medium | not high-confidence existing-group placement |
...
| `autochthonous_origin_from_mushrooms` | autochthonous origin from mushrooms | map_to_existing_group | `cosmic_origin` Cosmic Origin and Creation | medium | not high-confidence existing-group placement |
| `autonomous_or_enchanted_game_board` | autonomous or enchanted game board | map_to_existing_group | `sacred_treasures` Sacred Treasures and Power Objects | medium | not high-confidence existing-group placement |
...
| `avenging_expedition_for_a_wronged_sister` | Avenging expedition for a wronged sister | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
...
| `avenging_pursuit_and_beheading_of_deceiver` | avenging pursuit and beheading of deceiver | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
| `avoid_corrupting_contact` | avoid corrupting contact | map_to_existing_group | `sacred_law` Sacred Law and Social Order | medium | not high-confidence existing-group placement |
| `avoidance_of_extremes_as_path_of_happiness` | avoidance of extremes as path of happiness | map_to_existing_group | `sacred_knowledge` Sacred Knowledge and Wisdom | medium | not high-confidence existing-group placement |
...
| `awakening_call_at_dawn` | Awakening call at dawn | map_to_existing_group | `mystical_quest` Mystical Quest and Interior Journey | medium | not high-confidence existing-group placement |
...
| `backbiting_as_cannibalistic_abomination` | backbiting as cannibalistic abomination | map_to_existing_group | `sacred_law` Sacred Law and Social Order | medium | not high-confidence existing-group placement |
| `bad_omen_reversed_by_favorable_interpretation` | Bad omen reversed by favorable interpretation | map_to_existing_group | `dream_and_vision` Dream, Vision, and Altered Perception | medium | not high-confidence existing-group placement |
| `bad_rule_through_wealth_replacing_skill` | bad rule through wealth replacing skill | map_to_existing_group | `royal_legitimacy` Royal Legitimacy and Sacred Kingship | medium | not high-confidence existing-group placement |
| `baited_food_leads_to_capture` | baited food leads to capture | needs_human_review | ``  | low | low confidence |
| `balder_s_life_in_the_mistletoe` | Balder's life in the mistletoe | map_to_existing_group | `external_soul` External Soul and Life-Token | medium | not high-confidence existing-group placement |
| `banishment_of_afflictions_to_inaccessible_or_chthonic_containment_places` | Banishment of afflictions to inaccessible or chthonic containment places | map_to_existing_group | `sacrifice` Sacrifice and Offering | medium | not high-confidence existing-group placement |
| `banishment_of_an_invading_evil_from_the_body` | Banishment of an invading evil from the body | map_to_existing_group | `sacrifice` Sacrifice and Offering | medium | not high-confidence existing-group placement |
| `bardic_performance_nested_within_the_main_narrative` | bardic performance nested within the main narrative | map_to_existing_group | `storytelling_as_power` Storytelling and Narrative as Sacred Power | medium | not high-confidence existing-group placement |
...
| `beheading_of_defeated_enemies` | beheading of defeated enemies | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
...
| `benefactor_repaid_with_harm` | benefactor repaid with harm | map_to_existing_group | `sacred_exchange` Sacred Exchange and Reciprocity | medium | not high-confidence existing-group placement |
| `benefit_friends_and_harm_enemies` | benefit friends and harm enemies | needs_human_review | ``  | low | low confidence |
| `besieged_camp_protected_by_obedience_to_absent_leader` | besieged camp protected by obedience to absent leader | map_to_existing_group | `royal_legitimacy` Royal Legitimacy and Sacred Kingship | medium | not high-confidence existing-group placement |
| `best_not_to_be_born` | Best not to be born | map_to_existing_group | `lament_and_mourning` Lament, Mourning, and Tragic Recognition | medium | not high-confidence existing-group placement |
...
| `betrayal_of_duty_punished` | betrayal of duty punished | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
| `betrayal_of_the_fugitive_heir_by_a_kinsman` | betrayal of the fugitive heir by a kinsman | map_to_existing_group | `royal_legitimacy` Royal Legitimacy and Sacred Kingship | medium | not high-confidence existing-group placement |
...
| `betrayed_guests_make_a_doomed_last_stand` | betrayed guests make a doomed last stand | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
| `betrayed_safe_conduct` | betrayed safe-conduct | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
...
| `binding_injunction_or_prohibition_by_honour` | binding injunction or prohibition by honour | map_to_existing_group | `sacred_law` Sacred Law and Social Order | medium | not high-confidence existing-group placement |
| `bleeding_or_blood_distilling_plant` | bleeding or blood-distilling plant | map_to_existing_group | `death_and_transformation` Death, Rebirth, and Transformation | medium | not high-confidence existing-group placement |
| `blemishless_and_sinless_condition_of_inhabitants` | Blemishless and sinless condition of inhabitants | map_to_existing_group | `otherworld` Otherworld and Enchanted Realm | medium | not high-confidence existing-group placement |
| `blind_elder_deceived_by_simulated_weather` | blind elder deceived by simulated weather | map_to_existing_group | `trickster` Trickster and Boundary Crosser | medium | not high-confidence existing-group placement |
| `blind_poet_receives_identity_through_blindness` | blind poet receives identity through blindness | map_to_existing_group | `sacred_knowledge` Sacred Knowledge and Wisdom | medium | not high-confidence existing-group placement |
| `blinding_of_a_one_eyed_cannibal_giant_with_heated_spits` | Blinding of a one-eyed cannibal giant with heated spits | map_to_existing_group | `threshold_guardian` Threshold Guardian | medium | not high-confidence existing-group placement |
...
| `blood_betrays_concealed_killing` | blood betrays concealed killing | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
| `blood_breaks_enchantment` | blood breaks enchantment | map_to_existing_group | `shapeshifter` Shapeshifter and Transformation | medium | not high-confidence existing-group placement |
| `blood_changes_a_plant_s_color` | Blood changes a plant's color | map_to_existing_group | `death_and_transformation` Death, Rebirth, and Transformation | medium | not high-confidence existing-group placement |
...
| `blood_like_atmospheric_omen_or_marvel` | Blood-like atmospheric omen or marvel | map_to_existing_group | `dream_and_vision` Dream, Vision, and Altered Perception | medium | not high-confidence existing-group placement |
...
| `boast_answered_by_supernatural_opposition` | boast answered by supernatural opposition | map_to_existing_group | `pride_and_downfall` Pride, Hubris, and Downfall | medium | not high-confidence existing-group placement |
| `boast_counter_boast_and_vengeance_duel` | boast, counter-boast, and vengeance duel | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
...
| `bodily_omen_of_right_and_left_eye_throbbing` | Bodily omen of right and left eye throbbing | map_to_existing_group | `dream_and_vision` Dream, Vision, and Altered Perception | medium | not high-confidence existing-group placement |
| `bodily_penalty_mirrors_the_offending_body_part` | bodily penalty mirrors the offending body part | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
| `bodily_prostration_as_solemn_adoration` | Bodily prostration as solemn adoration | map_to_existing_group | `sacred_exchange` Sacred Exchange and Reciprocity | medium | not high-confidence existing-group placement |
| `bodily_restoration_after_injury` | bodily restoration after injury | map_to_existing_group | `death_and_transformation` Death, Rebirth, and Transformation | medium | not high-confidence existing-group placement |
...
| `body_diminishes_while_voice_remains` | body diminishes while voice remains | map_to_existing_group | `death_and_transformation` Death, Rebirth, and Transformation | medium | not high-confidence existing-group placement |
...
| `boon_extorted_from_a_captured_pursuer` | boon extorted from a captured pursuer | map_to_existing_group | `sacred_exchange` Sacred Exchange and Reciprocity | medium | not high-confidence existing-group placement |
| `boon_of_protection_granted_to_a_defeated_host` | boon of protection granted to a defeated host | map_to_existing_group | `sacred_exchange` Sacred Exchange and Reciprocity | medium | not high-confidence existing-group placement |
...
| `borrowed_arms_for_a_challenge` | Borrowed arms for a challenge | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
...
| `bounded_long_enchantment_with_prophesied_release_condition` | bounded long enchantment with prophesied release condition | map_to_existing_group | `otherworld` Otherworld and Enchanted Realm | medium | not high-confidence existing-group placement |
| `broken_truce_followed_by_failed_mass_attack` | broken truce followed by failed mass attack | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
...
| `burden_removed_and_heart_opened` | burden removed and heart opened | map_to_existing_group | `divine_intervention` Divine Intervention and Guidance | medium | not high-confidence existing-group placement |
...
| `burning_or_disposal_of_anthropomorphic_effigy_at_midsummer` | burning or disposal of anthropomorphic effigy at midsummer | map_to_existing_group | `sacred_time` Sacred Time and Ritual Cycle | medium | not high-confidence existing-group placement |
...
| `captive_as_foreshadowed_doom_of_captor` | captive as foreshadowed doom of captor | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
...
| `captive_master_smith_under_coercion` | Captive master smith under coercion | map_to_existing_group | `sacred_craft` Sacred Craft and Divinely Aided Construction | medium | not high-confidence existing-group placement |
| `captive_refuses_treasure_secret_under_torture` | captive refuses treasure secret under torture | map_to_existing_group | `sacred_treasures` Sacred Treasures and Power Objects | medium | not high-confidence existing-group placement |
...
| `captive_woman_lured_by_false_celebration` | captive woman lured by false celebration | map_to_existing_group | `trickster` Trickster and Boundary Crosser | medium | not high-confidence existing-group placement |
...
| `captives_reclaimed_after_killing_their_captors` | Captives reclaimed after killing their captors | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
...
| `captured_prey_pleads_for_release` | captured prey pleads for release | map_to_existing_group | `trickster` Trickster and Boundary Crosser | medium | not high-confidence existing-group placement |
| `caretaker_harmed_by_those_he_cares_for` | caretaker harmed by those he cares for | needs_human_review | ``  | low | low confidence |
| `castaway_survives_by_clinging_to_floating_wood_and_reaching_land` | castaway survives by clinging to floating wood and reaching land | map_to_existing_group | `sacred_waters` Sacred Waters and Passage Through Water | medium | not high-confidence existing-group placement |
...
| `celestial_omens_prompting_royal_action` | celestial omens prompting royal action | map_to_existing_group | `dream_and_vision` Dream, Vision, and Altered Perception | medium | not high-confidence existing-group placement |
...
| `celestial_performance_entrancing_human_guests` | Celestial performance entrancing human guests | map_to_existing_group | `otherworld` Otherworld and Enchanted Realm | medium | not high-confidence existing-group placement |
| `celestial_placement_of_a_slain_giant_s_eyes` | celestial placement of a slain giant's eyes | map_to_existing_group | `primordial_sacrifice` Primordial Sacrifice and Dismemberment | medium | not high-confidence existing-group placement |
...
| `celestial_reassurance_in_leaving_home` | Celestial reassurance in leaving home | map_to_existing_group | `hero_journey` The Hero's Journey | medium | not high-confidence existing-group placement |
...
| `chain_of_transferred_blame` | chain of transferred blame | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
| `challenge_cry_draws_a_royal_opponent_from_his_stronghold` | Challenge cry draws a royal opponent from his stronghold | map_to_existing_group | `sacred_combat` Sacred Combat and Ritual Warfare | medium | not high-confidence existing-group placement |
...
| `charioteer_as_decisive_martial_advantage` | charioteer as decisive martial advantage | map_to_existing_group | `divine_warrior` Divine Warrior and Sacred Combat | medium | not high-confidence existing-group placement |
...
| `charity_and_good_deeds_followed_by_heaven` | charity and good deeds followed by heaven | map_to_existing_group | `divine_judgment` Divine Judgment and Moral Reckoning | medium | not high-confidence existing-group placement |
...
_Showing first 500 of 690 review rows._
